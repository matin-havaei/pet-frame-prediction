...
pet_unet_paper.py
=================
U-Net Baseline for PET Tumor Simulation Completion
— Paper-ready version — matches cGAN style exactly —

Architecture  : Standard U-Net (Ronneberger et al. 2015)
                Input: N_CONTEXT frames (stacked) + time embedding injected at bottleneck
Time encoding : Sinusoidal embedding (same as cGAN / cVAE)
Loss          : L1 + SSIM
Split         : 7 train / 2 val / 1 test

Reference: Ronneberger O, Fischer P, Brox T. U-Net: Convolutional Networks
...
Run:
  python pet_unet_paper.py
  python pet_unet_paper.py --loocv
"""
...
class UNet(nn.Module):
    """Standard U-Net with sinusoidal time embedding injected at the bottleneck."""
    ...
    def forward(self, ctx, t):
        t_bias = self.time_proj(self.time_emb(t))   # (B, f*8)
        ...
        b = b + t_bias[:, :, None, None]             # time injection
...
def save_comparison_grid(ctx_frames, pred_frames, gt_frames,
    ...
    plt.suptitle(
        f"U-Net — {tumor_name}  |  First {N_CONTEXT} frames given → rest predicted",
        fontsize=11)
    plt.tight_layout()
...
def save_tac(ctx_mean, ctx_t, pred_mean, gt_mean, tgt_t, tumor_name, path):
    ...
    plt.plot(tgt_t,  pred_mean, '--', color='tomato',    lw=2.5,
             label='Predicted (U-Net)', zorder=4)
...
def train_one_fold(train_tumors, val_tumors, test_tumor,
    ...
    in_ch = N_CONTEXT * 3
    model = UNet(in_ch, 3, FEATURES, TIME_EMB_DIM).to(device)
...
def run_loocv(device):
    print("\n" + "═"*65)
    print("  LEAVE-ONE-OUT CROSS-VALIDATION — U-Net")
    print("═"*65)
    results = []
    ...
    paper_line = (
        f"In LOOCV across all {len(results)} tumors, U-Net achieved "
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'loocv_results_unet.csv')
...
def main():
    parser = argparse.ArgumentParser(description='U-Net — PET Tumor Prediction')
