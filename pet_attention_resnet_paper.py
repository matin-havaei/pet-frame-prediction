...
=============================
Attention ResNet for PET Tumor Simulation Completion
— Paper-ready version — matches cGAN style exactly —

Architecture  : ResNet encoder-decoder with attention gates on skip connections.
                Residual blocks replace double-conv blocks (Mokri et al. 2025).
Time encoding : Sinusoidal embedding injected at bottleneck
Loss          : L1 + SSIM
Split         : 7 train / 2 val / 1 test

Reference: Mokri M, et al. (2025). [direct published comparison — see paper]

Setup:
  pip install torch torchvision numpy pillow matplotlib tqdm scikit-image scipy

Run:
  python pet_attention_resnet_paper.py
  python pet_attention_resnet_paper.py --loocv
"""
...
class ResBlock(nn.Module):
    """Pre-activation residual block with instance norm."""
...
class ResDownBlock(nn.Module):
    """Strided conv downsampling + residual block."""
...
class AttentionResNet(nn.Module):
    """
    Encoder-decoder with ResNet blocks and attention gates.
    Mirrors Attention U-Net skip structure but replaces double-conv
    with residual blocks at every level.
    """
    def __init__(self, in_ch, out_ch=3, features=64, time_emb_dim=64):
        ...
        # Stem (no downsampling) — keeps full spatial resolution for skip
        self.stem = nn.Sequential(
            nn.Conv2d(in_ch, f, 3, 1, 1, bias=False),
            nn.InstanceNorm2d(f),
            nn.ReLU(inplace=True),
            ResBlock(f),
        )
        # Encoder (stride-2 each)
        self.enc2 = ResDownBlock(f,    f*2)
        self.enc3 = ResDownBlock(f*2,  f*4)
        self.enc4 = ResDownBlock(f*4,  f*8)

        # Bottleneck
        self.bottleneck = nn.Sequential(
            ResDownBlock(f*8, f*8),
            ResBlock(f*8),
        )

        # Attention gates
        # ag4: gating=bottleneck(f*8 @ 8), skip=e4(f*8 @ 8)
        self.ag4 = AttentionGate(f_g=f*8, f_x=f*8, f_int=f*4)
        # ag3: gating=d4(f*8 @ 8→up→16), skip=e3(f*4 @ 16)
        self.ag3 = AttentionGate(f_g=f*8, f_x=f*4, f_int=f*2)
        # ag2: gating=d3(f*4 @ 16→up→32), skip=e2(f*2 @ 32)
        self.ag2 = AttentionGate(f_g=f*4, f_x=f*2, f_int=f)
        # ag1: gating=d2(f*2 @ 32→up→64), skip=e1(f @ 64)
        self.ag1 = AttentionGate(f_g=f*2, f_x=f,   f_int=f//2)

        # Decoder — each dec block takes cat(up_out, attended_skip) as input
        # dec4: input=f*8(bottleneck) + f*8(e4_attended) = f*16 → output f*8
        self.up4  = nn.ConvTranspose2d(f*8,  f*8, 2, 2)
        self.dec4 = nn.Sequential(
            nn.Conv2d(f*16, f*8, 1, bias=False),   # channel reduction
            ResBlock(f*8),
        )
        # dec3: input=f*8(d4_up) + f*4(e3_attended) = f*12 → output f*4
        self.up3  = nn.ConvTranspose2d(f*8,  f*8, 2, 2)
        self.dec3 = nn.Sequential(
            nn.Conv2d(f*12, f*4, 1, bias=False),
            ResBlock(f*4),
        )
        # dec2: input=f*4(d3_up) + f*2(e2_attended) = f*6 → output f*2
        self.up2  = nn.ConvTranspose2d(f*4,  f*4, 2, 2)
        self.dec2 = nn.Sequential(
            nn.Conv2d(f*6,  f*2, 1, bias=False),
            ResBlock(f*2),
        )
        # dec1: input=f*2(d2_up) + f(e1_attended) = f*3 → output f
        self.up1  = nn.ConvTranspose2d(f*2,  f*2, 2, 2)
        self.dec1 = nn.Sequential(
            nn.Conv2d(f*3,  f,   1, bias=False),
            ResBlock(f),
        )
    ...
    def forward(self, ctx, t):
        ...
        # Encoder
        e1 = self.stem(ctx)     # f    × 64
        e2 = self.enc2(e1)      # f*2  × 32
        e3 = self.enc3(e2)      # f*4  × 16
        e4 = self.enc4(e3)      # f*8  ×  8

        # Bottleneck (stride-2 inside ResDownBlock → f*8 × 4)
        b  = self.bottleneck(e4)
        b  = b + t_bias[:, :, None, None]

        # Decoder level 4: upsample bottleneck 4→8, attend e4, decode
        b_up = self.up4(b)                              # f*8 × 8
        e4a  = self.ag4(b_up, e4)                       # f*8 × 8  (gate uses b_up as g)
        d4   = self.dec4(torch.cat([b_up, e4a], dim=1)) # f*8 × 8

        # Decoder level 3: upsample d4 8→16, attend e3, decode
        d4_up = self.up3(d4)                              # f*8 × 16
        e3a   = self.ag3(d4_up, e3)                       # f*4 × 16
        d3    = self.dec3(torch.cat([d4_up, e3a], dim=1)) # f*4 × 16

        # Decoder level 2: upsample d3 16→32, attend e2, decode
        d3_up = self.up2(d3)                              # f*4 × 32
        e2a   = self.ag2(d3_up, e2)                       # f*2 × 32
        d2    = self.dec2(torch.cat([d3_up, e2a], dim=1)) # f*2 × 32

        # Decoder level 1: upsample d2 32→64, attend e1, decode
        d2_up = self.up1(d2)                              # f*2 × 64
        e1a   = self.ag1(d2_up, e1)                       # f   × 64
        d1    = self.dec1(torch.cat([d2_up, e1a], dim=1)) # f   × 64
...
def save_comparison_grid(ctx_frames, pred_frames, gt_frames,
    ...
    plt.suptitle(
        f"Attention ResNet — {tumor_name}  |  First {N_CONTEXT} frames given → rest predicted",
        fontsize=11)
    plt.tight_layout()
...
def save_tac(ctx_mean, ctx_t, pred_mean, gt_mean, tgt_t, tumor_name, path):
    ...
    plt.plot(tgt_t,  pred_mean, '--', color='tomato',    lw=2.5,
             label='Predicted (Attention ResNet)', zorder=4)
...
def train_one_fold(train_tumors, val_tumors, test_tumor,
    ...
    in_ch = N_CONTEXT * 3
    model = AttentionResNet(in_ch, 3, FEATURES, TIME_EMB_DIM).to(device)

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nAttention ResNet params : {n_params:,}")
...
def run_loocv(device):
    print("\n" + "═"*65)
    print("  LEAVE-ONE-OUT CROSS-VALIDATION — Attention ResNet")
    print("═"*65)
    results = []
    ...
    print("\n" + "═"*65)
    print("  LOOCV SUMMARY — Attention ResNet")
    print("═"*65)
    ...
    paper_line = (
        f"In LOOCV across all {len(results)} tumors, Attention ResNet achieved "
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'loocv_results_attention_resnet.csv')
...
def main():
    parser = argparse.ArgumentParser(
        description='Attention ResNet — PET Tumor Prediction')
