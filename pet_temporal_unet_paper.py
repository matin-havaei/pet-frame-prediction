...
==========================
Temporal U-Net for PET Tumor Simulation Completion
— Paper-ready version — matches cGAN style exactly —

Role in paper : HYBRID BASELINE — bridges the gap between spatial-only U-Net
                and fully sequential ConvLSTM. Processes context frames through
                a U-Net encoder, aggregates temporal features at the bottleneck
                via an LSTM, then decodes with time-conditioned skip connections.

Architecture  : U-Net encoder per context frame (shared weights)
                → feature stack → LSTM across time steps at bottleneck
                → U-Net decoder conditioned on final LSTM hidden state + time emb
                → predicted frame
Time encoding : Sinusoidal embedding added to LSTM hidden state before decoding
Loss          : L1 + SSIM
...
Run:
  python pet_temporal_unet_paper.py
  python pet_temporal_unet_paper.py --loocv
"""
...
# Model
FEATURES     = 32      # U-Net base channels (smaller — LSTM adds params)
TIME_EMB_DIM = 64
LSTM_LAYERS  = 1

# Training
BATCH_SIZE   = 8       # lower than pair models — processes N_CONTEXT frames per sample
EPOCHS       = 1000
LR           = 2e-4
WEIGHT_DECAY = 1e-4
LAMBDA_L1    = 1.0
...
def setup_device():
    if torch.cuda.is_available():
        ...
        dev = torch.device('cpu'); print("No GPU found — running on CPU")
    return dev


# ═════════════════════════════════════════════════════════════════════════════
#  DATASET  — pair-level (same as U-Net / cVAE)
# ═════════════════════════════════════════════════════════════════════════════
    ...
    def __init__(self, tumor_names, data_root, subfolder,
        ...
        for tumor in tumor_names:
            ...
            for i in range(n_context, len(files)):
                self.samples.append({'ctx': ctx_f, 'ctx_t': ctx_t,
...
class TemporalUNet(nn.Module):
    """
    Shared-weight U-Net encoder processes each context frame independently.
    Bottleneck features (B, N, f*8, h, w) are flattened spatially and fed to
    an LSTM across the N time steps. The final LSTM hidden state is reshaped
    back to a spatial feature map and injected with the target time embedding
    before the U-Net decoder.

    At inference, skip connections come from the LAST context frame's encoder
    (most recent spatial context).

    Input:
      ctx   : (B, N, 3, H, W)   — N context frames
      t_norm: (B,)               — normalised target timepoint

    Output: (B, 3, H, W)
    """
    def __init__(self, in_ch=3, out_ch=3, features=32,
                 n_context=10, time_emb_dim=64, lstm_layers=1):
        super().__init__()
        f = features
        ...
        # After 4 pools: 64 → 4, feature map is f*8 × 4 × 4
        self.bottleneck_h = IMG_SIZE // (2**4)          # 4
        lstm_in = f * 8 * self.bottleneck_h * self.bottleneck_h
        self.lstm = nn.LSTM(lstm_in, lstm_in,
                            num_layers=lstm_layers, batch_first=True)
        self.lstm_norm = nn.LayerNorm(lstm_in)

        # ── Time conditioning ──────────────────────────────────────────
        self.time_emb  = SinusoidalTimeEmbedding(time_emb_dim)
        self.time_proj = nn.Linear(time_emb_dim, f*8)

        # ── Decoder (uses skip connections from LAST context frame) ───
        ...
        self.out_layer = nn.Sequential(nn.Conv2d(f, out_ch, 1), nn.Tanh())
        self._init_weights()
    ...
    def encode_frame(self, x):
        """Encode single frame, return (e1, e2, e3, e4, bottleneck)."""
        e1 = self.enc1(x)
        e2 = self.enc2(self.pool(e1))
        e3 = self.enc3(self.pool(e2))
        e4 = self.enc4(self.pool(e3))
        b  = self.pool(e4)          # f*8 × h × h
        return e1, e2, e3, e4, b

    def forward(self, ctx, t_norm):
        B, N, C, H, W = ctx.shape
        ...
        last_skips = None
        for i in range(N):
            e1, e2, e3, e4, b = self.encode_frame(ctx[:, i])
            bottleneck_seq.append(b.flatten(1))     # (B, lstm_in)
            if i == N - 1:
                last_skips = (e1, e2, e3, e4)       # keep last frame's skips

        # ── LSTM over bottleneck sequence ──────────────────────────────
        seq = torch.stack(bottleneck_seq, dim=1)    # (B, N, lstm_in)
        lstm_out, _ = self.lstm(seq)                # (B, N, lstm_in)
        final_h = self.lstm_norm(lstm_out[:, -1])   # (B, lstm_in) — last step

        # Reshape back to spatial (B, f*8, h, h)
        f8 = last_skips[3].shape[1]
        h  = self.bottleneck_h
        b_spatial = final_h.view(B, f8, h, h)

        # ── Inject target time ─────────────────────────────────────────
        t_bias = self.time_proj(self.time_emb(t_norm))   # (B, f*8)
        b_spatial = b_spatial + t_bias[:, :, None, None]

        # ── Decode with skip connections from last context frame ───────
        e1, e2, e3, e4 = last_skips
        d4 = self.dec4(torch.cat([self.up4(b_spatial), e4], dim=1))
...
def recon_loss(pred, target, lambda_l1=1.0, lambda_ssim=0.5):
    l1 = F.l1_loss(pred, target); ssim = ssim_loss_fn(pred, target)
...
# ═════════════════════════════════════════════════════════════════════════════
#  VISUALISATION  (identical pattern to all other models)
# ═════════════════════════════════════════════════════════════════════════════

def save_training_curves(history, path):
    fig, axes = plt.subplots(1, 2, figsize=(12,4))
    axes[0].plot(history['total'],    color='steelblue', lw=2, label='total (train)')
    axes[0].plot(history['val_loss'], color='tomato',    lw=2, linestyle='--', label='total (val)')
    axes[0].set_title('Total Loss — Train vs Validation'); axes[0].legend(); axes[0].grid(alpha=0.3)
    axes[1].plot(history['l1'],   color='seagreen', lw=2,   label='L1 (train)')
    axes[1].plot(history['ssim'], color='purple',   lw=1.5, linestyle=':', label='SSIM-loss (train)')
...
def save_comparison_grid(ctx_frames, pred_frames, gt_frames,
    ...
    axes[0,0].set_ylabel("Context", fontsize=9); axes[1,0].set_ylabel("Predicted", fontsize=9)
    axes[2,0].set_ylabel("Ground Truth", fontsize=9)
    plt.suptitle(f"Temporal U-Net — {tumor_name}  |  First {N_CONTEXT} frames → rest predicted", fontsize=11)
...
def save_tac(ctx_mean, ctx_t, pred_mean, gt_mean, tgt_t, tumor_name, path):
    ...
    plt.plot(tgt_t, pred_mean, '--', color='tomato',    lw=2.5, label='Predicted (Temporal U-Net)', zorder=4)
...
def save_metrics_plot(ssim_s, psnr_s, mae_s, times_min, path):
    ...
    for ax, scores, lbl, col in zip(axes, [ssim_s,psnr_s,mae_s],
        ...
        ax.plot(times_min, scores, color=col, lw=2); ax.set_xlabel('Time (min)')
...
def train_one_fold(train_tumors, val_tumors, test_tumor,
    ...
    model = TemporalUNet(in_ch=3, out_ch=3, features=FEATURES,
                         n_context=N_CONTEXT, time_emb_dim=TIME_EMB_DIM,
                         lstm_layers=LSTM_LAYERS).to(device)

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nTemporal U-Net params : {n_params:,}")
    ...
    for epoch in range(1, epochs+1):
        ...
        for ctx_f, _, t_norm, target in train_loader:
            ...
            pred = model(ctx_f, t_norm)
            loss, l1, ssim = recon_loss(pred, target, LAMBDA_L1, LAMBDA_SSIM)
            opt.zero_grad(); loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
            ep['total'].append(loss.item()); ep['l1'].append(l1); ep['ssim'].append(ssim)
        ...
        with torch.no_grad():
            for ctx_f, _, t_norm, target in val_loader:
                ctx_f  = ctx_f.to(device); t_norm = t_norm.to(device); target = target.to(device)
                pred = model(ctx_f, t_norm)
                loss, _, _ = recon_loss(pred, target, LAMBDA_L1, LAMBDA_SSIM)
                val_losses.append(loss.item())

        avg_val = np.mean(val_losses); history['val_loss'].append(avg_val)

        if avg_val < best_val:
            best_val = avg_val; patience_count = 0
            torch.save({'epoch':epoch,'model':model.state_dict(),
                        'opt':opt.state_dict(),'val_loss':avg_val,'history':history}, best_ckpt)
        else:
            patience_count += 1

        if epoch % 100 == 0:
            torch.save(model.state_dict(), os.path.join(out_dir, f'model_ep{epoch}.pth'))

        if epoch % 50 == 0 or epoch == 1:
            print(f"Ep {epoch:4d}/{epochs} | loss={history['total'][-1]:.4f}  "
                  f"val={avg_val:.4f}  patience={patience_count}/{PATIENCE}")
    ...
    print(f"\nLoaded best model: epoch {ckpt['epoch']}  val_loss={ckpt['val_loss']:.4f}")
    ...
    pred_arr, gt_arr, ctx_times_min, tgt_times_min = run_inference(model, device, test_tumor)
    ...
    if early_mask.sum() > 0 and late_mask.sum() > 0:
        ...
        print(f"  Mann-Whitney U: p={p_val:.4f}  {'✓ significant' if p_val<0.05 else '✗ not significant'}")
...
def run_loocv(device):
    print("\n" + "═"*65 + "\n  LEAVE-ONE-OUT CROSS-VALIDATION — Temporal U-Net\n" + "═"*65)
    results = []
    ...
    print("\n" + "═"*65 + "\n  LOOCV SUMMARY — Temporal U-Net\n" + "═"*65)
    ssims=[r['ssim_mean'] for r in results]; psnrs=[r['psnr_mean'] for r in results]
    ...
    paper_line = (f"In LOOCV across all {len(results)} tumors, Temporal U-Net achieved "
                  f"SSIM {np.mean(ssims):.3f}±{np.std(ssims):.3f}, "
                  f"PSNR {np.mean(psnrs):.2f}±{np.std(psnrs):.2f} dB, "
                  f"MAE {np.mean(maes):.4f}±{np.std(maes):.4f}.")
    print(f"\n  Paper sentence:\n  \"{paper_line}\"")
    csv_path = os.path.join(OUTPUT_DIR, 'loocv_results_temporal_unet.csv')
...
def main():
    parser = argparse.ArgumentParser(description='Temporal U-Net — PET Tumor Prediction')
