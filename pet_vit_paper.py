...
pet_vit_paper.py
================
Vision Transformer (ViT) for PET Tumor Simulation Completion
— Paper-ready version — matches cGAN style exactly —

Architecture  : ViT encoder (patch embedding → transformer blocks) processes
                the stacked context frames. A lightweight CNN decoder upsamples
                the token grid back to full resolution. Target timepoint is
                injected as a learned token prepended to the sequence (analogous
                to [CLS] token in BERT / ViT).

Design choices:
  • Patch size 8 → 64px image → 8×8 = 64 patches (manageable sequence length)
  • Time embedding prepended as extra token; transformer attends over time + patches
  • CNN decoder: reshape token grid → upsample with transposed convolutions
  • Same sinusoidal time embedding as all other models for consistency

Time encoding : Sinusoidal embedding added to the time token
Loss          : L1 + SSIM
Split         : 7 train / 2 val / 1 test

Reference: Dosovitskiy A, et al. An Image is Worth 16x16 Words: Transformers
...
Run:
  python pet_vit_paper.py
  python pet_vit_paper.py --loocv
"""
...
# Model
PATCH_SIZE   = 8       # 64/8 = 8 patches per side → 64 patches total
EMB_DIM      = 256     # transformer embedding dimension
N_HEADS      = 8       # attention heads
N_LAYERS     = 6       # transformer blocks
MLP_RATIO    = 4       # MLP hidden dim = EMB_DIM * MLP_RATIO
DROPOUT      = 0.1
TIME_EMB_DIM = 64      # sinusoidal time embedding dim (projected to EMB_DIM)

# Training
BATCH_SIZE   = 16
EPOCHS       = 1000
LR           = 1e-4    # lower LR typical for ViT
WEIGHT_DECAY = 1e-4
LAMBDA_L1    = 1.0
LAMBDA_SSIM  = 0.5
PATIENCE     = 150
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, emb_dim, n_heads, mlp_ratio=4, dropout=0.1):
        super().__init__()
        self.norm1 = nn.LayerNorm(emb_dim)
        self.attn  = nn.MultiheadAttention(emb_dim, n_heads,
...
class PatchEmbed(nn.Module):
    """
    Splits (B, C, H, W) into non-overlapping patches and projects each to emb_dim.
    Output: (B, n_patches, emb_dim)
    """
    ...
    def forward(self, x):
        # (B, C, H, W) → (B, emb_dim, H/P, W/P) → (B, n_patches, emb_dim)
        x = self.proj(x)
        B, D, H, W = x.shape
        return x.flatten(2).transpose(1, 2), H, W  # (B, HW, D), H, W


class CNNDecoder(nn.Module):
    """
    Reshapes the patch token grid back to spatial feature maps,
    then upsamples to full resolution with transposed convolutions.

    Input : (B, n_patches, emb_dim)  where n_patches = (H/P)*(W/P)
    Output: (B, 3, H, W)
    """
    def __init__(self, emb_dim, patch_size, img_size, out_ch=3):
        super().__init__()
        self.patch_size  = patch_size
        self.grid_size   = img_size // patch_size   # e.g. 64/8 = 8
        self.emb_dim     = emb_dim
        n_ups            = int(math.log2(patch_size))  # number of 2× upsamples
        ...
        for i in range(n_ups):
            is_last = (i == n_ups - 1)
            layers += [
                nn.ConvTranspose2d(ch_in, ch_out if not is_last else out_ch,
                                   4, 2, 1,
                                   bias=False if not is_last else True),
            ]
            if not is_last:
                layers += [nn.InstanceNorm2d(ch_out), nn.ReLU(inplace=True)]
            ch_in  = ch_out
            ch_out = max(ch_out // 2, out_ch)
        layers.append(nn.Tanh())
        self.net = nn.Sequential(*layers)

    def forward(self, tokens):
        # tokens: (B, n_patches, emb_dim)
        B = tokens.shape[0]
        g = self.grid_size
        # reshape to spatial feature map
        x = tokens.transpose(1, 2).view(B, self.emb_dim, g, g)
        return self.net(x)


class PETViT(nn.Module):
    """
    Vision Transformer for temporal PET frame prediction.

    Input:
      ctx_flat : (B, N*3, H, W)  — N context frames stacked channel-wise
      t_norm   : (B,)             — normalised target timepoint

    Pipeline:
      1. PatchEmbed ctx_flat → patch tokens (B, P, D)
      2. Add learned positional embedding
      3. Prepend time token (B, 1, D) ← sinusoidal time emb → D
      4. L transformer blocks
      5. Extract patch tokens (drop time token)
      6. CNN decoder → (B, 3, H, W)
    """
    def __init__(self, in_ch, img_size, patch_size,
                 emb_dim, n_heads, n_layers, mlp_ratio,
                 time_emb_dim, dropout=0.1):
        super().__init__()
        n_patches = (img_size // patch_size) ** 2

        self.patch_embed = PatchEmbed(in_ch, patch_size, emb_dim)
        self.pos_embed   = nn.Parameter(torch.zeros(1, n_patches, emb_dim))

        # Time token: sinusoidal → linear → emb_dim
        self.time_emb  = SinusoidalTimeEmbedding(time_emb_dim)
        self.time_proj = nn.Linear(time_emb_dim, emb_dim)

        self.blocks = nn.Sequential(*[
            TransformerBlock(emb_dim, n_heads, mlp_ratio, dropout)
            for _ in range(n_layers)
        ])
        self.norm = nn.LayerNorm(emb_dim)

        self.decoder = CNNDecoder(emb_dim, patch_size, img_size, out_ch=3)
    ...
    def forward(self, ctx_flat, t_norm):
        B = ctx_flat.shape[0]

        # 1. Patch embedding + positional encoding
        tokens, H, W = self.patch_embed(ctx_flat)    # (B, P, D)
        tokens = tokens + self.pos_embed              # broadcast

        # 2. Time token
        t_tok = self.time_proj(self.time_emb(t_norm))  # (B, D)
        t_tok = t_tok.unsqueeze(1)                       # (B, 1, D)

        # 3. Prepend time token → (B, 1+P, D)
        seq = torch.cat([t_tok, tokens], dim=1)

        # 4. Transformer
        seq = self.norm(self.blocks(seq))

        # 5. Drop time token, keep patch tokens
        patch_tokens = seq[:, 1:, :]                  # (B, P, D)

        # 6. CNN decoder
        return self.decoder(patch_tokens)              # (B, 3, H, W)
...
def save_comparison_grid(ctx_frames, pred_frames, gt_frames,
    ...
    plt.suptitle(
        f"ViT — {tumor_name}  |  First {N_CONTEXT} frames given → rest predicted",
        fontsize=11)
    plt.tight_layout()
...
def train_one_fold(train_tumors, val_tumors, test_tumor,
    ...
    in_ch = N_CONTEXT * 3
    model = PETViT(
        in_ch=in_ch, img_size=IMG_SIZE, patch_size=PATCH_SIZE,
        emb_dim=EMB_DIM, n_heads=N_HEADS, n_layers=N_LAYERS,
        mlp_ratio=MLP_RATIO, time_emb_dim=TIME_EMB_DIM, dropout=DROPOUT,
    ).to(device)

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nViT parameters    : {n_params:,}")
    print(f"Patch size        : {PATCH_SIZE}×{PATCH_SIZE}  →  "
          f"{(IMG_SIZE//PATCH_SIZE)**2} patches")
    print(f"Transformer depth : {N_LAYERS} blocks × {N_HEADS} heads")
    ...
    # Cosine warmup + decay (better for transformers than linear decay)
    warmup_steps = min(50, epochs // 10)
    def lr_lambda(epoch):
        if epoch < warmup_steps:
            return epoch / max(warmup_steps, 1)
        progress = (epoch - warmup_steps) / max(epochs - warmup_steps, 1)
...
def run_loocv(device):
    print("\n" + "═"*65)
    print("  LEAVE-ONE-OUT CROSS-VALIDATION — ViT")
    print("═"*65)
    results = []
    ...
    paper_line = (
        f"In LOOCV across all {len(results)} tumors, ViT achieved "
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'loocv_results_vit.csv')
...
def main():
    parser = argparse.ArgumentParser(description='ViT — PET Tumor Prediction')
